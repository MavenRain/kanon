...
report["em_dash_files"] = [str(path.relative_to(ROOT)) for path in ROOT.rglob("*.lean")
                          if ".lake" not in path.relative_to(ROOT).parts
                          and chr(0x2014) in path.read_text()]
report["pass"] = (not report["forbidden"] and not report["unexpected_proof_blocks"]
                  and not report["em_dash_files"])
(EVIDENCE / "source-audit.json").write_text(json.dumps(report, indent=2) + "\n")
