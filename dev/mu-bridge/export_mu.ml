(** Export a checked natural-family fixture to Lean raw syntax.
...
let main (args : string list) : (string, string) result =
  ...
  | [_exe; path] ->
      if Sys.file_exists path && not (Sys.is_directory path) then
        export (In_channel.with_open_bin path In_channel.input_all)
      else Error ("cannot read fixture " ^ path)
  | [] | [_] | _ :: _ :: _ :: _ -> Error "usage: export_mu FIXTURE.kan"
