...
After these bounded bridges, the next foundation work is to generalize declaration interpretation beyond the fixed families, extend it to open typed terms, and prove correspondence with the checker and its recursive-call rules. The category, admitted diagrams, existence assumptions, and substitution behavior must remain explicit. If extra existence or computation rules are needed, disclose them before claiming the strict primitive requirement has been met.
