...
The [dependent fusion increment](MU-FINITARY-ELIM-FUSION.md) constructs
constructor-preserving maps between displayed algebras over a fixed base.
...
The later [finite-family bridge](MU-FINITARY-BRIDGE.md) generalizes the declaration step to unindexed families with arbitrary finite constructor lists and direct recursive children. Validation retains the exact exported declaration and derives a finite signature, using the existing constructed initiality. Its [raw-term decoder](MU-FINITARY-DECODE.md) connects closed constructor terms to that signature with exact reconstruction certificates and roundtrip proofs. Indexed and mutual declarations, payload fields, open typed terms, and correspondence with the checker and its recursive-call rules remain the next foundation obligations. The category, admitted diagrams, existence assumptions, and substitution behavior must remain explicit. If extra existence or computation rules are needed, disclose them before claiming the strict primitive requirement has been met.
